    ...
    <motion.div variants={fadeInUp} whileHover={{ y: -10, rotateX: 5, rotateY: 5 }} className="group">
      <Card className="overflow-hidden bg-white shadow-lg hover:shadow-2xl transition-all duration-500 transform-gpu perspective-1000">
        <div className="relative overflow-hidden">
          <img
            src={`/images/${property.thumbnailImage}` || "/placeholder.svg?height=300&width=400"}
            alt={property.title}
          ...
          <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
            ...
            {/* Show wishlist button only if user is logged in */}
            {session && session.user && showWishlistButton && (
              <Button
        ...
        <CardContent className="p-6">
          ...
          <p className="text-2xl font-bold text-amber-600 mb-3">{property.type==="Plot"?`${property.cents} Cents = ${formatPrice(property.price)}` :formatPrice(property.price)}</p>

          <div className="flex items-center gap-4 text-slate-600 mb-4">
            {property.type!=="Plot" && <><div className="flex items-center gap-1">
