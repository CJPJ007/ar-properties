              ...
              >
                ...
                {session && session.user ? (
                  <div className="flex items-center gap-3">
                    ...
                    >
                      {session?.user?.image ? (
                        <Avatar className="w-8 h-8 border-4 border-white/20 dark:border-gray-700">
                          <AvatarImage
                            src={session.user.image || "/placeholder.svg"}
                          />
                          <AvatarFallback className="text-xl bg-white/20 text-white dark:text-gray-200 p-2">
                            {session.user.name?.charAt(0) || "U"}
                          </AvatarFallback>
                        </Avatar>
                      ) : (
                        <Avatar className="w-8 h-8 border-4 border-white/20 bg-white/20 dark:bg-gray-800 dark:border-gray-700">
                          <AvatarFallback className="text-xl text-white dark:text-gray-200">
                            {session?.user?.name?.charAt(0) || "U"}
                          </AvatarFallback>
                        </Avatar>
                      )}
